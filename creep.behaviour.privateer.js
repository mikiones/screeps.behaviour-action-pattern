module.exports = {
    ...
    exploitNextRoom: function(creep){
        ...
        let FlagDirs = FlagDir.filter(FLAG_COLOR.invade.exploit).map(f => Game.flags[f.name])
            .map( f =>
        { return {
            'spots':Game.rooms[f.pos.roomName] == null?1:Game.rooms[f.pos.roomName].sources.map(s => s.accessibleFields).reduce((a,b)=> a+b, 0),
            'name': f.name,
            'roomName:':f.pos.roomName,
            'targets': f.targetOf == null ? 0 : f.targetOf.length,
            'distance': Creep.action.travelling.getRoomPath(creep, f.pos).length
        }}).map(e => { e.left = e.spots - e.targets; return e;}).filter (e => e.left > 0)
